...
public final class MVCCManager {
    ...
    public func createSnapshot(tid: UInt64) -> Snapshot {
        lock.lock(); defer { lock.unlock() }
        return Snapshot(
            tid: tid,
            activeTIDs: Set(activeTIDs.keys),
            committedTIDs: Set(committedTIDs.keys),
            cutoffTID: tid
        )
    }
    
    public func getLatestVisibleVersion(table: String, rid: RID, snapshot: Snapshot) -> Version? {
        lock.lock(); defer { lock.unlock() }
        guard let chain = tableVersions[table]?[rid] else { return nil }
        return latestVisibleVersion(chain: chain, snapshot: snapshot, readerTID: snapshot.tid)
    }
    
    public func getAllVersions(table: String, rid: RID) -> [Version] {
        lock.lock(); defer { lock.unlock() }
        return tableVersions[table]?[rid] ?? []
    }
    
    public func purgeObsoleteVersions() {
        lock.lock(); defer { lock.unlock() }
        // Implementation would remove obsolete versions
