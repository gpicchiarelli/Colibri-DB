...
final class EndToEndIntegrationTests: XCTestCase {
    ...
    func testConcurrentTransactionsWithIsolation() async throws {
        ...
        try await mvcc.write(txId: tx1, key: Key("key1"), value: Value.string("value1"))
        
        // Transaction 2: Should NOT see uncommitted key1
        let tx2 = try await txManager.begin()
        let readValue = try await mvcc.read(txId: tx2, key: Key("key1"))
        ...
        let readValue2 = try await mvcc.read(txId: tx2, key: Key("key1"))
        ...
        let tx3 = try await txManager.begin()
        let readValue3 = try await mvcc.read(txId: tx3, key: Key("key1"))
    ...
    func testWALRecoveryIntegration() async throws {
        ...
        do {
            ...
            let flushedLSN = wal.getFlushedLSN()
        ...
        do {
            ...
            let currentLSN = wal.getCurrentLSN()
    ...
    func testFullDatabaseOperations() async throws {
        ...
        let txInsert = try await txManager.begin()
        try await mvcc.write(txId: txInsert, key: Key("users:1"), value: Value.string("Alice"))
        try await txManager.commitTransaction(txId: txInsert)
        
        // Simulate SELECT * FROM users WHERE id = 1
        let txSelect = try await txManager.begin()
        let result = try await mvcc.read(txId: txSelect, key: Key("users:1"))
        ...
        let txUpdate = try await txManager.begin()
        try await mvcc.write(txId: txUpdate, key: Key("users:1"), value: Value.string("Bob"))
        try await txManager.commitTransaction(txId: txUpdate)
        
        // Verify update
        let txVerify = try await txManager.begin()
        let updated = try await mvcc.read(txId: txVerify, key: Key("users:1"))
