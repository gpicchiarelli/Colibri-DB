...
final class IndexConformanceTests: XCTestCase {
    ...
    func testTotalOrderGuarantee() async throws {
        ...
        for indexInfo in indexTypes {
            if indexInfo.supportsOrder {
                ...
                // Scan and verify order - use inclusive range
                // Use a smaller range to avoid issues with string comparison
                let minKey = Value.string("00000000")
                let maxKey = Value.string("00000999")
                
                // Verify keys are comparable
                let key0 = Value.string("00000000")
                let key999 = Value.string("00000999")
                XCTAssertTrue(key0 <= key999, "\(indexInfo.name): String keys should be comparable")
    ...
    private func scanIndex(index: any IndexProtocol, minKey: Value, maxKey: Value) async throws -> [(Value, [RID])] {
        // Ensure minKey <= maxKey before scanning
        guard minKey <= maxKey else {
            return []
        }
