...
final class IndexContractTests: XCTestCase {
    ...
    private struct SeededRNG {
        ...
        mutating func nextValue() -> Value {
            ...
            switch kind {
            case 0:
                return .int(Int64(nextInt(in: -1000...1000)))
            case 1:
    ...
    func test_Index_Insert_Then_Seek_Returns_Last_Value() async throws {
        ...
        let indexes: [any Index] = [
            BTreeIndexWrapper(BTreeIndex()),
            ARTIndexWrapper(await ARTIndex()),
            HashIndexWrapper(await HashIndex(isUnique: false)),
            LSMTreeWrapper(await LSMTree()),
            SkipListWrapper(await SkipList())
        ]
